...
main() {
  ...
  # echo "${*:-%title% - %artist%}" | sed "s/%artist%/$artist/g;s/%title%/$title/g;s/%album%/$album/g"i | sed 's/&/\\&/g' | fold -w 50
  echo "${*:-%title% - %artist%}" | sed "s@%artist%@$artist@g;s@%title%@$title@g"i | sed 's/&/\\&/g' | fold -w 45
}
